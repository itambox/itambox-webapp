...
Maturity is declared per capability, not per module. The authoritative list, with each entry's activation mode and declared limitations, is the [capability registry](https://github.com/itambox/design-docs/blob/main/development/capability-registry.md); [module maturity](https://github.com/itambox/design-docs/blob/main/development/module-maturity.md) is the short reader's guide. Tenant Resource Grants are a Stable, security-critical capability with a dedicated [threat model](https://github.com/itambox/design-docs/blob/main/development/tenant-resource-grant-security.md). SCIM is Beta: tenant endpoints provision Users and expose Groups read-only, while provider-scoped endpoints provision both Users and provider-owned Groups.
