...
def _entity_actions(
    ...
    return [
        ...
        for path, base_value, local_value, remote_value in _path_triplets(
            section, identity, baseline, local, remote
        )
    ]
...
def _classify_path(
    ...
    if _equal(local, baseline) and not _equal(remote, baseline):
        return _action("update", identity, path, baseline, local, remote, decision="take_upstream", reason="upstream_change")
    if not _equal(local, baseline) and _equal(remote, baseline):
        return _action("unchanged", identity, path, baseline, local, remote, decision="keep_local", reason="local_override")
