...
class _WriteContext:
    ...
    def write_choice_sets(self) -> None:
        ...
        for item in self.definitions.get("choice_sets", []):
            ...
            if choice_set is None:
                ...
                _save_fields(
                    choice_set, {"label": item["label"], "lifecycle": item.get("lifecycle", "active")}, self.using
                )
    ...
    def write_fieldsets(self) -> None:
        ...
        for item in self.definitions.get("fieldsets", []):
            ...
            memberships = list(fieldset.field_memberships.using(self.using).order_by("position", "custom_field_id"))
    ...
    def write_asset_types(self) -> None:
        ...
        for item in self.definitions.get("asset_types", []):
            ...
            memberships = list(asset_type.fieldset_memberships.using(self.using).order_by("position", "fieldset_id"))
    ...
    def retire_missing(self) -> None:
        ...
        for queryset, section, identity in (
            ...
            (
                CustomFieldset.objects.using(self.using).filter(library_id=self.library.pk),
                "fieldsets",
                _object_identity,
            ),
            (
            ...
            (
                AssetType.all_objects.using(self.using).filter(library_id=self.library.pk),
                "asset_types",
                _asset_identity,
            ),
        ):
...
class _ReadContext:
    ...
    def read_asset_types(self) -> None:
        for asset_type in (
            self.library.asset_types.all()
            .select_related("manufacturer", "category")
            .prefetch_related("fieldset_memberships__fieldset")
        ):
