...
from ..models import AuditSession, AssetAudit, Asset, StatusLabel
from ..forms.audit_forms import AuditSessionForm, AssetAuditForm, AuditBarcodeScanForm
from ..filters import AuditSessionFilterSet
from ..forms.filter_forms import AuditSessionFilterForm
from ..reconciliation import audit_asset, close_audit_session, rehome_audit_session_mismatches

User = get_user_model()

class AuditSessionTable(BaseTable):
    pk = ToggleColumn(accessor='pk')
    name = tables.LinkColumn('assets:auditsession_detail', args=[A('pk')], verbose_name='Name')
...
class AuditSessionListView(ObjectListView):
    ...
    template_name = 'generic/object_list.html'  # Use standard list template to render Table nicely
    action_buttons = ('add',)
...
class AuditSessionDetailView(ObjectDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Calculate real-time campaign checklist metrics
...
class AssetAuditScanView(View):
    ...
    Validates scanned serials/tags, registers audits, and updates DOM checklist partials.
    """
...
class AuditSessionCloseView(GenericTransactionView):
    ...
    model_form = AuditSessionCloseForm  # Simple confirmation form
...
class AuditSessionDeleteView(ObjectDeleteView):
    ...
    success_url = reverse_lazy('assets:auditsession_list')
