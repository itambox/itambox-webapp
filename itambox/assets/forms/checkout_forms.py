...
class AssetCheckOutForm(forms.Form):
    ...
    target_type = forms.ChoiceField(
        choices=TARGET_CHOICES, widget=forms.Select(attrs={"class": "form-select"}), label=_("Assign to")
    )
    ...
    notes = forms.CharField(
        widget=forms.Textarea(attrs={"rows": 3, "class": "form-control"}), required=False, label=_("Notes")
    )
        ...
        if asset:
            ...
            if asset.tenant:
                ...
                self.fields["location"].queryset = (
                    Location.objects.filter(tenant=asset.tenant).select_related("site").order_by("site__name", "name")
                )
...
class AssetCheckInForm(forms.Form):
    ...
    checkin_date = forms.DateField(
        required=False, widget=forms.DateInput(attrs={"type": "date", "class": "form-control"}), label=_("Checkin Date")
    )
    notes = forms.CharField(
        widget=forms.Textarea(attrs={"rows": 3, "class": "form-control"}), required=False, label=_("Notes")
    )
