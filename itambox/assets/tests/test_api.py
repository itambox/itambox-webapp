...
class ITAMBoxAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.holder_staff = AssetHolder.objects.create(
            user=self.staff, first_name="Staff", last_name="User", upn="staff@tenant-a.com", tenant=self.tenant_a
        )

        self.holder_a = AssetHolder.objects.create(
            first_name="Holder", last_name="A", upn="holder@tenant-a.com", tenant=self.tenant_a
        )
        self.holder_b = AssetHolder.objects.create(
            first_name="Holder", last_name="B", upn="holder@tenant-b.com", tenant=self.tenant_b
        )

        # Staging Site and Locations
        self.site = Site.objects.create(name="HQ", slug="hq")
        self.location_a = Location.objects.create(
            name="Staging A", slug="staging-a", site=self.site, tenant=self.tenant_a
        )
        self.location_b = Location.objects.create(
            name="Staging B", slug="staging-b", site=self.site, tenant=self.tenant_b
        )
        ...
        self.software = Software.objects.create(name="Office 2021", manufacturer=self.manufacturer)

        self.license_a = License.objects.create(
            name="Office 2021 Standard", software=self.software, seats=10, tenant=self.tenant_a
        )
    ...
    def test_checkout_multiple_targets_fails(self):
        ...
        checkout_url = reverse("api:assets_api:asset-checkout", kwargs={"pk": self.asset_a.pk})
        data = {"holder_id": self.holder_a.id, "location_id": self.location_a.id, "notes": "Should fail"}
    ...
    def test_license_and_assignment_crud_parity(self):
        ...
        data = {"name": "API Office License", "software_id": self.software.id, "seats": 50}
        ...
        assign_data = {"license_id": license_id, "asset_id": self.asset_a.id}
    ...
    def test_asset_checkout_and_checkin_actions_with_custom_status_location_and_date(self):
        ...
        deployed_status = StatusLabel.objects.create(
            name="Assigned-InUse", slug="assigned-inuse", type=StatusLabel.TYPE_DEPLOYED
        )
        returned_status = StatusLabel.objects.create(
            name="Staging-NeedsInspect", slug="staging-needsinspect", type=StatusLabel.TYPE_PENDING
        )
        return_location = Location.objects.create(
            name="Storage Closet 2", slug="storage-closet-2", site=self.site, tenant=self.tenant_a
        )
