...
class AssetAssignmentTestCase(TestCase):
    ...
    def test_checkout_asset_to_asset(self):
        ...
        """
        target = checkout_asset(asset=self.peripheral_monitor, asset_target=self.host_laptop, user=self.user)
    ...
    def test_checkin_asset_from_asset(self):
        ...
        """
        checkout_asset(asset=self.peripheral_monitor, asset_target=self.host_laptop, user=self.user)
...
class AssetTagSequenceTestCase(TestCase):
    def setUp(self):
        ...
        self.asset_type_laptop = AssetType.objects.create(
            manufacturer=self.manufacturer, model="Laptop Model", category=self.category_laptop
        )
        self.asset_type_server = AssetType.objects.create(
            manufacturer=self.manufacturer, model="Server Model", category=self.category_server
        )
    ...
    def test_sequence_resolution_hierarchy(self):
        ...
        AssetTagSequence.all_objects.create(
            prefix="GLAP-", category=self.category_laptop, next_value=200, zero_padding=3
        )
        ...
        AssetTagSequence.all_objects.create(
            tenant=self.tenant_a, category=self.category_laptop, prefix="TALAP-", next_value=400, zero_padding=3
        )

        # --- Test 1: Tenant A + Category Laptop -> should match TALAP-400 ---
        asset_1 = Asset.objects.create(
            name="Laptop 1", status=self.status, tenant=self.tenant_a, asset_type=self.asset_type_laptop
        )
        ...
        asset_2 = Asset.objects.create(
            name="Server 1", status=self.status, tenant=self.tenant_a, asset_type=self.asset_type_server
        )
        ...
        asset_3 = Asset.objects.create(
            name="Laptop 2", status=self.status, tenant=self.tenant_b, asset_type=self.asset_type_laptop
        )
        ...
        asset_4 = Asset.objects.create(
            name="Server 2", status=self.status, tenant=self.tenant_b, asset_type=self.asset_type_server
        )
    ...
    def test_form_requires_asset_tag_and_suggests_it(self):
        ...
        seq = AssetTagSequence.all_objects.create(
