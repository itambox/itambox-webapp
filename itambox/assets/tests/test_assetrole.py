...
class IsModularPropertyTests(TenantTestMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.status = StatusLabel.objects.create(name="Active", slug="active-ismod", type=StatusLabel.TYPE_DEPLOYABLE)
    ...
    def test_is_modular_true_when_role_allows_components(self):
        role = AssetRole.objects.create(name="Server", slug="server-ismod", allows_components=True)
        asset = self._make_asset(role)
        self.assertTrue(asset.is_modular)

    def test_is_modular_false_when_role_does_not_allow(self):
        role = AssetRole.objects.create(name="Phone", slug="phone-ismod", allows_components=False)
        asset = self._make_asset(role)
