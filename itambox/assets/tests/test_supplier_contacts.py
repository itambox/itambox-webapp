...
class SupplierContactModelTest(TenantTestMixin, TestCase):
    def setUp(self):
        self.setup_tenant_context(permissions=["assets.view_supplier", "assets.change_supplier"])
...
class SupplierSerializerContactsTest(TenantTestMixin, TestCase):
    def setUp(self):
        self.setup_tenant_context(permissions=["assets.view_supplier"])
        self.supplier = Supplier.objects.create(
            name="Beta Corp",
            slug="beta-corp",
        )
        ...
        self.supplier_ct = ContentType.objects.get_for_model(Supplier)
        self.contact = Contact.objects.create(name="Dana Smith", email="dana@beta.example.com", phone="+49-30-1234567")
