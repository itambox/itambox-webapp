...
def managed_form_data(rows, *, initial_forms=0):
    ...
    for index, row in enumerate(rows):
        ...
        data[f"{prefix}-scope_group"] = row.get("scope_group").pk if row.get("scope_group") else ""
        data[f"{prefix}-assigned_tenants"] = [tenant.pk for tenant in row.get("tenants", [])]
...
class UserGroupFormTests(TestCase):
    ...
    def test_form_creates_one_grant_with_own_and_managed_scopes(self):
        ...
        self.assertTrue(group.group_memberships.filter(membership=self.member).exists())
        self.assertEqual(role_grant.reason, "")
        self.assertIsNone(role_grant.valid_until)
