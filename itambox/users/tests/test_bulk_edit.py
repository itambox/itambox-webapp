...
class UserBulkEditTests(TestCase):
    ...
    def test_non_superuser_cannot_grant_staff_or_superuser(self):
        ...
        self.assertIn(
            "Only superusers can grant or modify staff, superuser, or login status.", response.content.decode("utf-8")
        )
    ...
    def test_self_lockout_guard_active(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "You cannot deactivate your own user account in a bulk edit operation.", response.content.decode("utf-8")
        )
    ...
    def test_self_lockout_guard_superuser(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "You cannot revoke your own superuser status in a bulk edit operation.", response.content.decode("utf-8")
        )
