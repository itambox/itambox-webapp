...
def _live_role_grant_scope_request(
    ...
    *,
    ...
    if grant.membership_id:
        ...
        if not principal.is_active or (principal.deleted_at is not None and principal.pk != restoring_user_group_id):
            return None
        principal_tenant = principal.tenant
    else:
        return None
    ...
    for scope in grant.scopes.all():
        if scope.scope_type == "own":
            has_own = role.tenant_id == principal_tenant.pk or (
                grant.membership_id
            ...
            has_live_group_scope = (
                has_live_group_scope
                or Tenant._base_manager.filter(
