...
class RolePresetFieldMapTests(TenantTestMixin, TestCase):
    ...
    def test_administrator_is_full_matrix(self):
        ...
        expected = {f"perm_{key}_{action}" for key in MATRIX_MODELS for action in ("read", "create", "edit", "delete")}
    ...
    def test_technician_excludes_delete(self):
        ...
        expected = {f"perm_{key}_{action}" for key in MATRIX_MODELS for action in ("read", "create", "edit")}
    ...
    def test_field_map_scoped_to_forms_matrix_models(self):
        ...
        valid_fields = {
            f"perm_{key}_{action}" for key in MATRIX_MODELS for action in ("read", "create", "edit", "delete")
        }
