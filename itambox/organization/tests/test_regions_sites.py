...
from organization.models import Location, Region, Site, SiteGroup
...
class RegionViewTests(TestCase):
    ...
    def test_edit_view_post(self):
        ...
        response = self.client.post(url, {"name": "North America Updated", "slug": "north-america-updated"})
        self.assertEqual(response.status_code, 302)
...
class SiteViewTests(TestCase):
    ...
    def test_list_view_counts(self):
        ...
        asset_type = AssetType.objects.create(manufacturer=manufacturer, model="Latitude 5520", slug="latitude-5520")
        ...
        Asset.objects.create(
            name="Asset 1", asset_tag="TAG-1", asset_type=asset_type, asset_role=role, status=status, location=loc1
        )
        Asset.objects.create(
            name="Asset 2", asset_tag="TAG-2", asset_type=asset_type, asset_role=role, status=status, location=loc2
        )
        Asset.objects.create(
            name="Asset 3", asset_tag="TAG-3", asset_type=asset_type, asset_role=role, status=status, location=loc2
        )
    ...
    def test_create_view_post(self):
        ...
        response = self.client.post(url, {"name": "Branch Office", "slug": "branch-office", "status": "active"})
    ...
    def test_edit_view_post(self):
        ...
        response = self.client.post(url, {"name": "HQ Office Renamed", "slug": "hq-office-renamed", "status": "active"})
        if response.status_code != 302:
            form = response.context.get("form")
            if form:
                self.fail(f"Form invalid. Errors: {form.errors}")
...
class LocationViewTests(TestCase):
    ...
    def test_create_view_post(self):
        ...
        response = self.client.post(
            url, {"name": "Network Closet", "slug": "network-closet", "site": self.site.pk, "status": "active"}
        )
        if response.status_code != 302:
            form = response.context.get("form")
            if form:
    ...
    def test_edit_view_post(self):
        ...
        response = self.client.post(
            url, {"name": "Server Room 2", "slug": "server-room-2", "site": self.site.pk, "status": "active"}
        )
        if response.status_code != 302:
            form = response.context.get("form")
            if form:
...
class SiteGroupViewExpansionTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="siteadmin", password="testpassword", is_staff=True, is_superuser=True
        )
        self.client.force_login(self.user)
