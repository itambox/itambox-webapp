...
class MembershipCreateAuthzTests(TestCase):
    def setUp(self):
        ...
        self.victim = User.objects.create_user(
            username="authz_victim",
            email="victim@authz.test",
            password="pw",
        )
    ...
    def test_tampered_tenant_with_new_email_writes_nothing_to_b(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            Membership.objects.filter(tenant=self.tenant_b).filter(user__email="brand_new@authz.test").exists()
        )
    ...
    def test_authorized_admin_get_and_post_succeed(self):
        ...
        self.assertEqual(
            response.status_code, 302, getattr(response, "context", None) and response.context["form"].errors.as_json()
        )
...
class MembershipFormOracleDefenseTests(TestCase):
    ...
    def setUp(self):
        ...
        self.member = User.objects.create_user(
            username="oracle_member",
            email="member@oracle.test",
            password="pw",
        )
        Membership.objects.create(user=self.member, tenant=self.tenant, is_active=True)
        self.outsider = User.objects.create_user(
            username="oracle_outsider",
            email="out@oracle.test",
            password="pw",
        )
    ...
    def test_unauthorized_actor_gets_a_non_revealing_error(self):
        form = MembershipForm(
            data=membership_post_data(
                ...
                new_user_first_name="M",
                new_user_last_name="Ember",
            ),
            tenant=self.tenant,
            user=self.outsider,
        )
