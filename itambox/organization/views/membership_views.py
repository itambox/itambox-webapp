...
class MembershipListView(ObjectListView):
    ...
    def _render_outside_access_panel(self, request):
        ...
        return render(
            request,
            "organization/memberships/outside_access_panel.html",
            {
                "tenant": tenant,
                "entries": entries,
            },
...
class MembershipDetailView(ObjectDetailView):
    queryset = Membership.objects.select_related("user", "tenant").prefetch_related(
        "role_grants__role",
...
class MembershipSendResetView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if not (
            request.user.is_superuser or request.user.has_perm("organization.change_membership", obj=membership.tenant)
        ):
            messages.error(
                request,
...
class MembershipCreateView(_MembershipFormViewMixin, ObjectEditView):
    ...
    def _authorized_tenant(self):
        ...
        authorized = None
        if tenant is not None and (user.is_superuser or user.has_perm("organization.add_membership", obj=tenant)):
            authorized = tenant
