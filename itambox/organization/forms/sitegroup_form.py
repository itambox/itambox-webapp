...
class SiteGroupForm(forms.ModelForm):
    parent = forms.ModelChoiceField(
        queryset=SiteGroup.objects.all(), required=False, widget=forms.Select(attrs={"class": "form-select"})
    )
    ...
    def clean_parent(self):
        ...
        if parent and self.instance and self.instance.pk and parent.pk == self.instance.pk:
            raise forms.ValidationError(_("A site group cannot be its own parent."))
        return parent
