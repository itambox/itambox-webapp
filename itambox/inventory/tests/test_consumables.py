...
class ConsumableAssignmentModelTests(TestCase):
    def setUp(self):
        ...
        self.asset_type = AssetType.objects.create(
            manufacturer=self.manufacturer, model="OptiPlex 7010", slug="optiplex-7010"
        )
        self.asset = Asset.objects.create(
            name="OptiPlex Desktop", asset_tag="OPTIPLEX-001", asset_type=self.asset_type, asset_role=self.asset_role
        )
...
class ConsumableCheckoutViewTests(TestCase):
    ...
    def test_checkout_validation_insufficient_stock(self):
        url = reverse("inventory:consumable_checkout", kwargs={"pk": self.consumable.pk})
        response = self.client.post(
            url, {"target_type": "asset", "assigned_asset": self.asset.pk, "from_location": self.location.pk, "qty": 10}
        )
        self.assertEqual(response.status_code, 200)
