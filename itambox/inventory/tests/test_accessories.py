...
class AccessoryAssignmentModelTests(TestCase):
    def setUp(self):
        ...
        self.asset = Asset.objects.create(
            name="Dell Latitude", asset_tag="DELL-001", asset_type=self.asset_type, asset_role=self.asset_role
        )
...
class AccessoryCheckoutViewTests(TestCase):
    ...
    def test_checkout_to_holder_success(self):
        ...
        response = self.client.post(
            url,
            {"target_type": "holder", "assigned_holder": self.holder.pk, "from_location": self.location.pk, "qty": 1},
        )
        self.assertEqual(response.status_code, 302)
        assignment = AccessoryAssignment.objects.get(accessory=self.accessory)
    ...
    def test_checkout_validation_insufficient_stock(self):
        url = reverse("inventory:accessory_checkout", kwargs={"pk": self.accessory.pk})
        response = self.client.post(
            url, {"target_type": "asset", "assigned_asset": self.asset.pk, "from_location": self.location.pk, "qty": 15}
        )
        self.assertEqual(response.status_code, 200)
...
class AccessoryStockFilterSetTests(TestCase):
    def setUp(self):
        ...
        self.acc2 = Accessory.objects.create(
            name="Wireless Keyboard", slug="wireless-keyboard", manufacturer=self.manufacturer
        )
