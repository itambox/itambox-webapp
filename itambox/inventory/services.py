    ...
    **kwargs: Any,
    ...
    with transaction.atomic():
        ...
        if source_location:
            ...
            loc_stock = (
                stock_model._base_manager.filter(**{item_field: item, "location": source_location}).aggregate(
                    qty=Sum("qty")
                )["qty"]
                or 0
            )
...
def resolve_grant_for_checkout(item, item_field, stock_model, assignment_model, source_location, user=None):
