...
def compile_legacy_report_context(template, active_tenant=None, filter_tenants=None):
    ...
    if template.report_type == ReportTemplate.REPORT_TYPE_ASSET_SUMMARY:
        ...
        for asset in assets_qs[:500]:
            ...
            if "warranty_months" in active_cols:
                ...
                _active_warranty = next(
                    (
                        w
                        for w in asset.warranties.all()
                        if w.deleted_at is None and w.start_date and w.end_date and w.start_date <= _today <= w.end_date
                    ),
                    None,
                )
                months = (
                    int((_active_warranty.end_date - _active_warranty.start_date).days / 30.4)
                    if _active_warranty
                    else None
                )
        ...
        if template.include_summary_cards:
            summary_cards = [{"label": _("Total License Products"), "value": str(total_licenses)}]
        ...
        for lic in licenses_qs[:500]:
            ...
            row["_group_by"] = (
                lic.software.name if (template.group_by_field == "software" and lic.software) else "General"
            )
            rows.append(row)
        ...
        if not rows:
            ...
            rows.append(row)
            if template.include_summary_cards:
                summary_cards = [{"label": _("Total License Products"), "value": "1 (Mock)"}]
            total_assigned = 42
            total_seats = 50
        ...
        subs_qs = Subscription.objects.filter(deleted_at__isnull=True, status="active").select_related(
            "provider", "tenant"
        )
        if filter_tenants:
        ...
        for sub in subs_qs[:500]:
            ...
            row["_group_by"] = (
                sub.provider.name if (template.group_by_field == "provider" and sub.provider) else "General"
            )
            rows.append(row)
        ...
        if not rows:
            ...
            row["_group_by"] = (
                ...
                if template.group_by_field == "maintenance_type"
                else 'MacBook Pro 16"'
                if template.group_by_field == "asset"
                else "General"
            )
            rows.append(row)
            if template.include_summary_cards:
        ...
        for asset in assets_qs[:500]:
            ...
            if "depreciation_months" in active_cols:
                months = (
                    asset.asset_type.depreciation.months
        ...
        if report_tenant_ids is not None:
            ...
            license_count_expr = Count(
                "licenses",
                filter=Q(licenses__deleted_at__isnull=True),
                distinct=True,
            )
        ...
        if not rows:
            ...
            rows.append(row)
            if template.include_summary_cards:
                summary_cards = [{"label": _("Total Software Products"), "value": "1 (Mock)"}]
        ...
        expiring_soon_count = active_contracts_qs.filter(end_date__gte=today, end_date__lte=in_30).count()
        ...
        for c in contracts_qs:
            ...
            supplier_costs[(supplier_name, cur)] = supplier_costs.get((supplier_name, cur), 0.0) + annual_cost
        ...
        if not rows:
            ...
            row["_group_by"] = (
                ...
                if template.group_by_field == "contract_type"
                else "Acme Corp"
                if template.group_by_field == "contract_supplier"
                else "General"
            )
            rows.append(row)
            if template.include_summary_cards:
        ...
        if not rows:
            ...
            row["_group_by"] = (
                "Recycle / WEEE"
                if template.group_by_field == "disposal_method"
                else "NIST Purge (cryptographic or ATA Secure Erase)"
                if template.group_by_field == "disposal_sanitization_method"
                else "WEEE Compliant"
                if template.group_by_field == "disposal_weee_compliant"
                else "General"
            )
            rows.append(row)
            if template.include_summary_cards:
        ...
        hw_models = [
            (
                _("Accessory"),
                _hw_scope(Accessory.objects.filter(deleted_at__isnull=True).select_related("manufacturer", "category")),
            ),
            (
                _("Consumable"),
                _hw_scope(
                    Consumable.objects.filter(deleted_at__isnull=True).select_related("manufacturer", "category")
                ),
            ),
            (
                _("Component"),
                _hw_scope(Component.objects.filter(deleted_at__isnull=True).select_related("manufacturer", "category")),
            ),
        ]
        ...
        if not rows:
            ...
            row["_group_by"] = (
                ...
                if template.group_by_field == "custody_signature_provider"
                else "1.0"
                if template.group_by_field == "custody_eula_version"
                else "General"
            )
            rows.append(row)
            if template.include_summary_cards:
