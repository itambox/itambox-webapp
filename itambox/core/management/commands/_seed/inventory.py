    ...
    def _seed_inventory_stock(self):
        ...
        for slug in self._tenants:
            ...
            for acc_slug in random.sample(list(self._accessories), k=4):
                ...
                AccessoryStock.objects.get_or_create(
                    accessory=self._accessories[acc_slug], location=loc, defaults={"qty": qty}
                )
                stock_count += 1
            for con_slug in random.sample(list(self._consumables), k=2):
                ConsumableStock.objects.get_or_create(
                    consumable=self._consumables[con_slug],
                    location=loc,
        ...
        for comp_slug, comp in self._components.items():
            ComponentStock.objects.get_or_create(
                component=comp, location=msp_loc, defaults={"qty": random.choice([2, 4, 5, 8, 12, 0])}
            )  # one 0 → low-stock signal
            comp_count += 1
        ...
        for tslug in self._tenants:
            ...
            for comp_slug in random.sample(list(self._components), k=random.randint(2, 4)):
                ComponentStock.objects.get_or_create(
                    component=self._components[comp_slug], location=loc, defaults={"qty": random.choice([1, 2, 3, 4])}
                )
                comp_count += 1
