...
def register_schedule(func, *, defaults=None, using=DEFAULT_DB_ALIAS):
    ...
    try:
        ...
        with transaction.atomic(using=using):
            ...
            existing = list(schedules.select_for_update().filter(func=func).order_by("id"))
            if existing:
                schedule = existing[0]
            ...
            return schedules.create(func=func, **defaults)
