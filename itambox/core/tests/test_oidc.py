...
@override_settings(ITAMBOX_TENANT_OIDC_CONFIGS=TEST_OIDC_CONFIGS)
class TenantOIDCTestCase(TestCase):
    ...
    def test_assetholder_provisioning_and_linking(self):
        ...
        existing_holder = AssetHolder.objects.create(
            first_name="Pre", last_name="Existing", upn="pre@alpha.com", email="pre@alpha.com", tenant=self.tenant_alpha
        )
    ...
    def test_group_to_role_synchronization(self):
        ...
        self.assertTrue(
            any("delete_" in p for p in membership_admin.role_grants.first().role.permissions if "dashboard" not in p)
        )
        ...
        self.assertFalse(
            any("delete_" in p for p in membership_mgr.role_grants.first().role.permissions if "dashboard" not in p)
        )
    ...
    def test_existing_customer_user_transition_normalizes_to_provider_identity(self):
        ...
        self.assertTrue(
            UserGroup._base_manager.filter(
        ...
        self.assertTrue(
            AssetHolder._base_manager.filter(
    ...
    def test_callback_view_tenant_routing(self, mock_authenticate, mock_get_settings):
        ...
        session["oidc_states"] = {"state-123": {"nonce": "nonce-123", "code_verifier": "verifier-123"}}
        ...
        mock_get_settings.side_effect = lambda attr, *args: (
            "/" if attr == "OIDC_REDIRECT_OK" else (args[0] if args else None)
        )
    ...
    def test_upn_and_onetoone_collisions_multitenant(self):
        ...
        self.assertEqual(
            user_a.asset_holder_profiles.filter(tenant=self.tenant_alpha).first().tenant, self.tenant_alpha
        )
    ...
    def test_callback_view_invalid_session_tenant_slug(self):
        ...
        session = self.client.session
        session["oidc_states"] = {"state-456": {"nonce": "nonce-456", "code_verifier": "verifier-456"}}
