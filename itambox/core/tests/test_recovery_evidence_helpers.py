...
class RecoveryEvidenceHelperTests(SimpleTestCase):
    @staticmethod
    def _probe_objects(*, token_tenant_id=7, webhook_tenant_id=7, token_expired=False):
    ...
    def test_command_reads_explicit_canaries_and_emits_json(self):
        ...
        patches = {name: patch.object(command_module, name, create=True) for name in model_names}
        with (
