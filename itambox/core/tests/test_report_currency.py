...
class MixedCurrencyReportTests(TenantTestMixin, TestCase):
    ...
    def test_asset_depreciation_book_value_per_currency(self):
        _, _rows, cards, *_ = self._compile(
            ReportTemplate.REPORT_TYPE_ASSET_DEPRECIATION, ["asset_tag", "purchase_cost", "current_value"]
        )
    ...
    def test_asset_maintenance_cost_per_currency(self):
        AssetMaintenance.objects.create(
            asset=self.asset_eur, cost=Decimal("100.00"), currency="EUR", start_date=self.asset_eur.purchase_date
        )
        AssetMaintenance.objects.create(
            asset=self.asset_usd, cost=Decimal("300.00"), currency="USD", start_date=self.asset_usd.purchase_date
        )
        _, _rows, cards, *_ = self._compile(
            ReportTemplate.REPORT_TYPE_ASSET_MAINTENANCE, ["maintenance_asset", "maintenance_cost"]
        )
