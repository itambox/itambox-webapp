...
class SchemaEvidenceNormalizationTests(SimpleTestCase):
    def test_index_name_is_removed_without_losing_definition(self):
        ...
        self.assertEqual(
            canonical,
            "CREATE UNIQUE INDEX ON public.assets_asset USING btree (asset_tag) WHERE (deleted_at IS NULL)",
        )
        self.assertNotIn("legacy_random_name", canonical)

    def test_quoted_index_name_is_removed(self):
        definition = 'CREATE INDEX "legacy index" ON public.assets_asset USING btree (asset_tag)'
    ...
    def test_non_array_casts_are_not_canonicalized(self):
        definition = "CHECK ((status = 'active'::character varying))"
    ...
    def test_command_emits_canonical_catalog_and_preserves_multiplicity(
        ...
        cursor.fetchall.side_effect = [
            ...
            [
                ("assets_asset", "constraint_old_name", "UNIQUE (asset_tag)", False),
            ],
            [
                (
                    "assets_asset",
                    "legacy_index_a",
                    "CREATE INDEX legacy_index_a ON public.assets_asset USING btree (asset_tag)",
                    True,
                    True,
                    True,
                ),
                (
                    "assets_asset",
                    "legacy_index_b",
                    "CREATE INDEX legacy_index_b ON public.assets_asset USING btree (asset_tag)",
                    True,
                    True,
                    True,
                ),
