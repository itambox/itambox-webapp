...
class ExtrasListWiringTests(TenantTestMixin, TestCase):
    ...
    def test_list_views_declare_filter_and_checkbox(self):
        ...
        cases = [
            (views.AlertRuleListView, AlertRuleFilterSet, AlertRuleFilterForm, ["alert_type", "severity", "is_active"]),
            (
                views.NotificationChannelListView,
                NotificationChannelFilterSet,
                NotificationChannelFilterForm,
                ["channel_type", "enabled"],
            ),
            (
                views.ReportTemplateListView,
                ReportTemplateFilterSet,
                ReportTemplateFilterForm,
                ["report_type", "style_preset"],
            ),
            (
                views.ScheduledReportListView,
                ScheduledReportFilterSet,
                ScheduledReportFilterForm,
                ["report", "frequency", "format", "is_active"],
            ),
        ]
    ...
    def test_report_template_filterset_search_narrows(self):
        ...
        with self.tenant_context(self.tenant):
            ReportTemplate.objects.create(
                name="Alpha Report", report_type=ReportTemplate.REPORT_TYPE_ASSET_SUMMARY, tenant=self.tenant
            )
            ReportTemplate.objects.create(
                name="Beta Report", report_type=ReportTemplate.REPORT_TYPE_ASSET_SUMMARY, tenant=self.tenant
            )
            matched = ReportTemplateFilterSet({"q": "Alpha"}, queryset=ReportTemplate.objects.all()).qs
