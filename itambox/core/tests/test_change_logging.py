...
class ChangeLoggingTestCase(TestCase):
    ...
    def test_asset_checkout_status_change_logged(self):
        ...
        available_status, _ = StatusLabel.objects.get_or_create(
            slug="available", defaults={"name": "Available", "type": "deployable", "color": "28a745"}
        )
        in_use_status, _ = StatusLabel.objects.get_or_create(
            slug="in-use", defaults={"name": "In Use", "type": "deployed", "color": "007bff"}
        )

        role = AssetRole.objects.create(name="Test Role", slug="test-role")
        asset = Asset.objects.create(
            name="Test Checkout Laptop", asset_tag="TAG-CHK-TEST", status=available_status, asset_role=role
        )
    ...
    def test_objectchange_tracking(self):
        ...
        self.assertTrue(
            isinstance(change_create.get_changed_object_url(), str) or change_create.get_changed_object_url() is None
        )
