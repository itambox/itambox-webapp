...
from core.integrity import (
    ...
    CLASS_TENANT_GROUP,
    TenantTopology,
    check_cross_tenant_assignments,
    check_custody_receipts,
    check_license_seats,
    check_location_site_tenants,
    check_null_tenants,
    check_purchase_orders,
    check_stock_tenant_conflicts,
    run_all_checks,
)
...
def _build_msp_topology():
    ...
    return SimpleNamespace(
        ...
        group_a=group_a,
        group_b=group_b,
        tenant_group_a=tenant_group_a,
        tenant_group_b=tenant_group_b,
        unrelated=unrelated,
    )
...
class TenantTopologyClassifyTests(TestCase):
    ...
    def test_same_tenant(self):
        self.assertEqual(
            self.topo.classify(self.t.provider.pk, self.t.provider.pk),
            CLASS_SAME_TENANT,
        )

    def test_provider_to_managed_both_directions(self):
        self.assertEqual(
            self.topo.classify(self.t.provider.pk, self.t.managed.pk),
            CLASS_PROVIDER_MANAGED,
        )
        self.assertEqual(
            self.topo.classify(self.t.managed.pk, self.t.provider.pk),
            CLASS_PROVIDER_MANAGED,
        )
    ...
    def test_unrelated_invalid(self):
        self.assertEqual(
            self.topo.classify(self.t.unrelated.pk, self.t.provider.pk),
            CLASS_INVALID,
        )
        self.assertEqual(
            self.topo.classify(self.t.unrelated.pk, self.t.tenant_group_a.pk),
            CLASS_INVALID,
        )
...
class CheckCrossTenantAssignmentsTests(TestCase):
    ...
    def test_provider_to_managed_checkout_yields_finding_and_grant_proposal(self):
        ...
        holder = AssetHolder.objects.create(
            first_name="Man",
            last_name="Aged",
            upn="man.aged@managed.example.com",
        ...
        matching_proposals = [
            p
            for p in proposals
            if p.resource_model == "inventory.AccessoryStock"
            and p.item_id == accessory.pk
            and p.location_id == from_location.pk
            and p.grantee_tenant_id == self.t.managed.pk
        ]
    ...
    def test_unrelated_tenants_yield_finding_but_no_proposal(self):
        ...
        holder = AssetHolder.objects.create(
            first_name="Other",
            last_name="Holder",
            upn="other.holder@provider.example.com",
            tenant=self.t.provider,
        )
        # AbstractAssignment.clean() (ADR-0001 phase 4) forbids constructing a
        # cross-tenant row without a resource grant; create same-tenant first
    ...
    def test_proposal_dedup_across_assignments_to_same_grantee(self):
        ...
        holder1 = AssetHolder.objects.create(
            first_name="One",
            last_name="Managed",
            upn="one.managed@managed.example.com",
            tenant=self.t.managed,
        )
        holder2 = AssetHolder.objects.create(
            first_name="Two",
            last_name="Managed",
            upn="two.managed@managed.example.com",
            tenant=self.t.managed,
        )
        # AbstractAssignment.clean() (ADR-0001 phase 4) forbids constructing a
        # cross-tenant row without a resource grant; create same-tenant first
    ...
    def test_asset_assignment_cross_tenant_flagged(self):
        holder_a = AssetHolder.objects.create(
            first_name="Own",
            last_name="Tenant",
            upn="own.tenant@provider.example.com",
            tenant=self.t.provider,
        )
        ...
        matches = [f for f in findings if f.model == "assets.AssetAssignment" and f.pk == assignment.pk]
        self.assertEqual(len(matches), 1)
...
class CheckPurchaseOrdersTests(TestCase):
    ...
    def test_line_tenant_mismatch_flagged(self):
        ...
        po = PurchaseOrder.objects.create(
            tenant=self.t.provider,
            order_number="PO-LINE-MISMATCH-1",
            supplier=self.supplier,
            destination_location=destination,
        )
        ...
        findings = check_purchase_orders()
        matches = [f for f in findings if f.check == "po_line_tenant_mismatch" and f.pk == line.pk]
        self.assertEqual(len(matches), 1)
    ...
    def test_line_item_tenant_mismatch_flagged(self):
        ...
        po = PurchaseOrder.objects.create(
            tenant=self.t.provider,
            order_number="PO-ITEM-MISMATCH-1",
            supplier=self.supplier,
            destination_location=destination,
        )
        accessory = Accessory.objects.create(
            name="PO Item Accessory",
            manufacturer=self.mfr,
            tenant=self.t.managed,
        )
        line = PurchaseOrderLine.objects.create(
        ...
        findings = check_purchase_orders()
        matches = [f for f in findings if f.check == "po_line_item_tenant_mismatch" and f.pk == line.pk]
        self.assertEqual(len(matches), 1)
    ...
    def test_global_line_item_not_flagged(self):
        ...
        po = PurchaseOrder.objects.create(
            tenant=self.t.provider,
            order_number="PO-GLOBAL-ITEM-1",
            supplier=self.supplier,
            destination_location=destination,
        )
        accessory = Accessory.objects.create(
            name="PO Global Accessory",
            manufacturer=self.mfr,
            tenant=None,
        )
        line = PurchaseOrderLine.objects.create(
...
class CheckLicenseSeatsTests(TestCase):
    ...
    def test_cross_tenant_holder_seat_flagged(self):
        ...
        holder = AssetHolder.objects.create(
            first_name="Seat",
            last_name="Holder",
            upn="seat.holder@managed.example.com",
            tenant=self.t.managed,
        )
    ...
    def test_same_tenant_seat_not_flagged(self):
        ...
        holder = AssetHolder.objects.create(
            first_name="Local",
            last_name="Holder",
            upn="local.holder@provider.example.com",
            tenant=self.t.provider,
        )
        seat = LicenseSeatAssignment.objects.create(license=license_, assigned_holder=holder)
...
class CheckCustodyReceiptsTests(TestCase):
    ...
    def test_cross_tenant_holder_flagged(self):
        ...
        holder = AssetHolder.objects.create(
            first_name="Custody",
            last_name="Holder",
            upn="custody.holder@managed.example.com",
            tenant=self.t.managed,
        )
    ...
    def test_same_tenant_not_flagged(self):
        ...
        holder = AssetHolder.objects.create(
            first_name="Local",
            last_name="Custody",
            upn="local.custody@provider.example.com",
            tenant=self.t.provider,
        )
        receipt = CustodyReceipt.objects.create(asset=asset, holder=holder)
