...
class TenantIsolationTests(TenantTestMixin, TestCase):
    ...
    def test_licenses_scoped_to_active_tenant(self):
        ...
        baker.make(
            License, software=sw, tenant=self.tenant_b, seats=100, license_type=LicenseTypeChoices.PERPETUAL_SEAT
        )
    ...
    def test_reconcile_tenant_licensing_excludes_other_tenant_data(self):
        ...
        baker.make(
            License, software=sw_b, tenant=self.tenant_b, seats=3, license_type=LicenseTypeChoices.PERPETUAL_SEAT
        )
