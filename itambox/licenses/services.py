    ...
    **kwargs,
    ...
    with transaction.atomic():
        ...
        assignment = LicenseSeatAssignment.objects.create(
            license=lic, asset=asset, assigned_holder=assigned_holder, notes=notes
        )
...
def transfer_license_seat(
    assignment: LicenseSeatAssignment, new_license: License, user=None, request=None, **kwargs
) -> LicenseSeatAssignment:
    ...
    with transaction.atomic():
        ...
        if duplicate.exists():
            raise ValidationError(_("%(target)s already holds a seat on this license.") % {"target": target})
