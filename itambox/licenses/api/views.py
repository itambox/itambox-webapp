...
from .serializers import LicenseSeatAssignmentSerializer, LicenseSerializer


class LicenseViewSet(ITAMBoxModelViewSet):
    permission_classes = [TokenPermissions, StrictTenantPermission]
    queryset = License.objects.with_counts().select_related("software__manufacturer").prefetch_related("tags").all()
...
class LicenseSeatAssignmentViewSet(ITAMBoxModelViewSet):
    permission_classes = [TokenPermissions, StrictTenantPermission]
    queryset = LicenseSeatAssignment.objects.select_related("license__software", "asset", "assigned_holder").all()
