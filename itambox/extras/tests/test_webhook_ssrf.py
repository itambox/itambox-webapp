...
class WebhookEndpointSSRFTests(APITestCase):
    ...
    def setUp(self):
        self.admin = User.objects.create_user(username="wh_admin", password="pw", is_staff=True, is_superuser=True)
    ...
    def test_rejects_internal_target_urls(self):
        for target in (
            ...
            resp = self.client.post(self.url, {"name": f"wh-{target[:18]}", "target_url": target}, format="json")
    ...
    def test_accepts_external_url(self):
        ...
        resp = self.client.post(self.url, {"name": "wh-ok", "target_url": "http://8.8.8.8/hook"}, format="json")
