...
class DashboardWidgetsMultiTenancyTests(TestCase):
    def setUp(self):
        ...
        self.maint_a = AssetMaintenance.objects.create(asset=self.asset_a, start_date=date.today(), cost=150.00)
        ...
        self.sw = Software.objects.create(name="Office", manufacturer=self.mfr)
        self.lic_a = License.objects.create(name="Office 365 A", seats=10, software=self.sw, tenant=self.tenant_a)
        self.lic_b = License.objects.create(name="Office 365 B", seats=20, software=self.sw, tenant=self.tenant_b)
    ...
    def test_tenant_spend_widget_mixed_currencies(self):
        ...
        Asset.objects.create(
            name="Asset A USD",
            asset_tag="TAG-A-USD2",
            serial_number="SN-A-USD2",
            asset_type=self.asset_type,
            status=self.status,
            tenant=self.tenant_a,
    ...
    def test_low_stock_widget_scoping(self):
        ...
        con_a = Consumable.objects.create(name="Toner A", manufacturer=self.mfr, min_qty=3, tenant=self.tenant_a)
