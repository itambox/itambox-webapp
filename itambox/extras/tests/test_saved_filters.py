...
class SavedFilterVisibilityTests(TenantTestMixin, TestCase):
    def setUp(self):
        ...
        self.user_a2 = User.objects.create_user(username="user_a2", email="a2@example.com", password="password")
        ...
        self.sf_a_shared = SavedFilter.objects.create(
            name="A shared",
            content_type=self.ct_tag,
            tenant=self.tenant,
            shared=True,
            created_by=self.user_a2,
            parameters={"q": "x"},
        )
        ...
        self.sf_a_other_model = SavedFilter.objects.create(
            name="A custom-field filter",
            content_type=self.ct_cf,
            tenant=self.tenant,
            shared=True,
            created_by=self.tenant_user,
            parameters={"q": "cf"},
