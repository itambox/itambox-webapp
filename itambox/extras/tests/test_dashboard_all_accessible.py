...
class DashboardContextMixin:
    ...
    def _request(self, user, *, tenant=None, group=None, membership=None, all_accessible=False):
...
def _make_catalog():
    ...
    status = StatusLabel.objects.create(
        name="I133 Deployable",
        slug="i133-deployable",
        type="deployable",
    )
...
class AllAccessibleWidgetTests(DashboardContextMixin, TestCase):
    ...
    def setUp(self):
        ...
        self._seed(self.tenant_b, key="b", display="BravoAsset", cost=2000, maint=300, sub_cost=800, seats=20)
        self._seed(self.tenant_c, key="c", display="ZuluSecretAsset", cost=9999, maint=999, sub_cost=999, seats=99)

    def _seed(self, tenant, *, key, display, cost, maint, sub_cost, seats):
        asset = Asset.objects.create(
            name=display,
            asset_tag=f"I133-TAG-{key}",
            serial_number=f"I133-SN-{key}",
...
class TenantSpendWidgetTests(DashboardContextMixin, TestCase):
    ...
    def _asset(self, tenant, key, cost):
        return Asset.objects.create(
            name=f"Spend {key}",
            asset_tag=f"I133TS-{key}",
            serial_number=f"I133TS-SN-{key}",
            asset_type=self.asset_type,
            status=self.status,
            tenant=tenant,
            purchase_cost=cost,
...
class GetScopedQuerysetScopeTests(DashboardContextMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.inaccessible = Tenant.objects.create(
            name="I133 Inaccessible",
            slug="i133g-inacc",
        )
        self.soft_deleted = Tenant.objects.create(
            name="I133 SoftDeleted",
            slug="i133g-soft",
            managed_by=self.provider,
        )
    ...
    def _asset(self, tenant, key):
        return Asset.objects.create(
            name=f"Asset {key}",
            asset_tag=f"I133G-{key}",
            serial_number=f"I133G-SN-{key}",
            asset_type=self.asset_type,
            status=self.status,
            tenant=tenant,
            purchase_cost=100,
    ...
    def test_tenant_group_scope_aggregates_accessible_group_tenants(self):
        ...
        self.assertEqual(
            ids,
            {
                self.assets["direct"].pk,
                self.assets["managed"].pk,
    ...
    def setUp(self):
        ...
        for tenant, key, cost in (
            ...
            Asset.objects.create(
                name=key,
                asset_tag=f"I133V-{key}",
                serial_number=f"I133V-SN-{key}",
                asset_type=self.asset_type,
                status=self.status,
                tenant=tenant,
                purchase_cost=cost,
