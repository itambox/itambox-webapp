...
class JournalEntryTenantIsolationAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.superuser = User.objects.create_user(username="je_super", password="pw", is_staff=True, is_superuser=True)
        perms = [
        ...
        grant(self.staff_a, self.tenant_a, role_a)
        ...
        grant(self.staff_a2, self.tenant_a, role_a)
        ...
        self.status = StatusLabel.objects.create(name="JE-Ready", slug="je-ready", type=StatusLabel.TYPE_DEPLOYABLE)
        ...
        self.entry_a = JournalEntry.objects.create(
            content_object=self.asset_a,
            user=self.staff_a,
            comment="A note",
        )
        self.entry_b = JournalEntry.objects.create(
            content_object=self.asset_b,
            user=self.superuser,
            comment="B note",
        )
