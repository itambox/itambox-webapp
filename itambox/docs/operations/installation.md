...
| `ITAMBOX_FIELD_ENCRYPTION_KEYS` | Strongly recommended — fallback-supported when unset. When unset, the key is derived from `ITAMBOX_SECRET_KEY` (development convenience; rotating the secret then makes encrypted fields unreadable). An explicitly supplied value that is not a valid keyring **is startup-blocking**. |
| `ITAMBOX_API_TOKEN_PEPPERS` | Strongly recommended — fallback-supported when unset. When unset, token hashing falls back to a `SECRET_KEY`-derived pepper (loudly warned). An explicitly supplied value that is not a valid mapping **is startup-blocking** — malformed configuration never silently falls back. |
...
| `ITAMBOX_API_TOKEN_PEPPERS` ⚠️ | **Strongly recommended** (prod) | *(derived from SECRET_KEY)* | Server-side peppers used to HMAC-hash API tokens at rest. Must be a JSON object whose numeric keys are rotation IDs and values are dedicated secrets of ≥50 characters. The highest ID peppers new tokens; older IDs keep existing tokens valid. Example: `{"1":"replace-with-a-random-secret-of-at-least-50-characters"}`. When **unset**, falls back to a `SECRET_KEY`-derived pepper with a loud production warning — acceptable for development only; tokens hashed under the fallback stop validating once a dedicated mapping is configured (re-issue them). An explicitly supplied value that is not a valid mapping **aborts production startup**; malformed configuration never silently falls back. |
