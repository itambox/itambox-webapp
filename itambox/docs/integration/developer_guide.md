...
The GraphQL endpoint is `/graphql/`. Authenticated users can open the built-in GraphiQL UI with a GET request, including in production; unauthenticated users are redirected to login. POST requests accept the same `Authorization: Token ***` header as REST. GraphiQL's React/GraphiQL/Explorer assets are copied into `static/dist/vendor/graphiql/` by `npm run build:vendor`, so the UI has no runtime CDN dependency. Schema introspection is available only with development settings; production keeps the schema itself non-introspectable.
