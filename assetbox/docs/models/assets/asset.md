...
### 3. Depreciation
AssetBox calculates monthly straight-line depreciation values dynamically using the parent asset type's **Depreciation Profile**, automatically deducting value based on months held between the `Purchase Date` and the current date, down to the defined `Salvage Value`.
