...
def _isolate_migration_test(method):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        popen_kwargs = {'cwd': os.getcwd(), 'env': child_env, 'stdout': subprocess.PIPE, 'stderr': subprocess.STDOUT, 'text': True, 'encoding': 'utf-8', 'errors': 'replace'}
        if os.name == 'nt':
...
class CoreVocabularyMigrationTests(TransactionTestCase):
    ...
    predecessor = [('assets', '0116_assettypeimagestage'), ('extras', '0120_issue479_t07_provenance_cutover')]
    target = [('assets', '0117_issue479_final_core_vocabulary'), ('extras', '0120_issue479_t07_provenance_cutover')]
    ...
    def _oracle():
        ...
        oracle_path = repository_root / 'scripts' / 'tests' / 'fixtures' / 'specification_vocabulary' / 'canonical-target.json'
    ...
    def _assert_exact_vocabulary(self, apps):
        ...
        core_fields = list(CustomField._base_manager.filter(namespace='itambox', management_kind='core').order_by('name'))
        self.assertEqual({field.name for field in core_fields}, expected_field_names)
        scope_models = {'asset_type': {('assets', 'assettype')}, 'asset': {('assets', 'asset')}, 'both': {('assets', 'asset'), ('assets', 'assettype')}}
        for expected in expected_fields:
            ...
            if expected['choice_set'] is not None:
                ...
                self.assertEqual((choice_set.namespace, choice_set.slug), ('itambox', expected['choice_set'].split('/', 1)[1]))
            validation = expected['validation']
            self.assertEqual(None if validation.get('minimum') is None else Decimal(validation['minimum']), field.minimum_value)
            self.assertEqual(None if validation.get('maximum') is None else Decimal(validation['maximum']), field.maximum_value)
            ...
            actual_types = {(content_type.app_label, content_type.model) for content_type in ContentTypeModel._base_manager.filter(pk__in=field.object_types.values_list('pk', flat=True))}
            scope_key = 'asset_type' if 'asset_type' in expected['targets'] and len(expected['targets']) == 1 else 'both' if len(expected['targets']) == 2 else 'asset'
        ...
        for expected in oracle['sections']:
            ...
            actual_memberships = list(FieldsetField._base_manager.filter(fieldset_id=fieldset.pk).order_by('position').values_list('custom_field__name', 'position'))
            expected_memberships = [(membership['field'].rsplit('/', 1)[1], membership['position']) for membership in expected['memberships']]
        ...
        for expected in oracle['choice_sets']:
            ...
            actual_choices = list(Choice._base_manager.filter(choice_set_id=choice_set.pk).order_by('position').values_list('key', 'label', 'lifecycle', 'position'))
            expected_choices = [(choice['key'], choice['label'], choice['lifecycle'], choice['position']) for choice in expected['choices']]
        ...
        for expected in oracle['categories']:
            ...
            self.assertEqual(sorted((key for key, value in category.applies_to.items() if value)), sorted(expected['applies_to']))
            actual_defaults = list(CategoryDefault._base_manager.filter(category_id=category.pk).order_by('position').values_list('fieldset__namespace', 'fieldset__slug', 'position'))
            expected_defaults = [(fieldset['fieldset'].split('/', 1)[0], fieldset['fieldset'].split('/', 1)[1], fieldset['position']) for fieldset in expected['default_fieldsets']]
    ...
    def test_supported_predecessor_upgrade_preserves_values_and_local_ownership(self):
        ...
        local_field = CustomField._base_manager.create(name='local_keep', namespace='local', label='Keep local field', field_type='text', activation='composed', management_kind='local', lifecycle='active')
        local_fieldset = CustomFieldset._base_manager.create(namespace='local', slug='local-keep', label='Keep local fieldset', description='Local definition must survive the core reconciliation.', management_kind='local', lifecycle='active')
        CustomFieldsetField._base_manager.create(fieldset_id=local_fieldset.pk, custom_field_id=local_field.pk, position=10)
        local_choice_set = ChoiceSet._base_manager.create(namespace='local', slug='local-keep', label='Keep local choices', management_kind='local', lifecycle='active')
        local_choice = Choice._base_manager.create(choice_set_id=local_choice_set.pk, key='local_value', label='Local value', position=10, lifecycle='active')
        local_category = Category._base_manager.create(name='Keep Local Category', slug='local-keep', description='Local category must survive.', applies_to={'component': True})
        CategoryDefault._base_manager.create(category_id=local_category.pk, fieldset_id=local_fieldset.pk, position=10)
        laptops = Category._base_manager.create(name='Old Laptop', slug='laptops', description='Old description', applies_to={'asset': True, 'component': True})
        ...
        asset_type = AssetType._base_manager.create(manufacturer_id=manufacturer.pk, model='Existing Model', slug='existing-model', custom_field_data={'input_voltage': '230.0', 'storage_medium': 'nvme_ssd', 'storage_interface': 'SSD-interface', 'poe_port_count': 8})
        old_asset_type_membership = apps.get_model('assets', 'AssetTypeFieldset')._base_manager.create(asset_type_id=asset_type.pk, fieldset_id=old_storage.pk, position=10)
        apps = self._migrate(self.target)
        self._assert_exact_vocabulary(apps)
        migrated_asset_type = apps.get_model('assets', 'AssetType')._base_manager.get(pk=asset_type.pk)
        self.assertEqual(migrated_asset_type.custom_field_data, {'input_voltage': '230.0', 'storage_medium': 'nvme_ssd', 'storage_interface': 'SSD-interface', 'poe_port_count': 8})
        self.assertEqual(apps.get_model('assets', 'AssetTypeFieldset')._base_manager.get(pk=old_asset_type_membership.pk).position, 10)
        ...
        self.assertTrue(apps.get_model('extras', 'CustomField')._base_manager.filter(pk=local_field.pk, namespace='local').exists())
        self.assertTrue(apps.get_model('extras', 'CustomFieldset')._base_manager.filter(pk=local_fieldset.pk, namespace='local').exists())
        self.assertTrue(apps.get_model('extras', 'CustomFieldChoiceSet')._base_manager.filter(pk=local_choice_set.pk, namespace='local').exists())
        self.assertTrue(apps.get_model('extras', 'CustomFieldChoice')._base_manager.filter(pk=local_choice.pk, choice_set_id=local_choice_set.pk).exists())
        self.assertTrue(apps.get_model('assets', 'Category')._base_manager.filter(pk=local_category.pk, slug='local-keep').exists())
        self.assertEqual(list(apps.get_model('assets', 'CategoryDefaultFieldset')._base_manager.filter(category_id=local_category.pk).values_list('fieldset_id', 'position')), [(local_fieldset.pk, 10)])
