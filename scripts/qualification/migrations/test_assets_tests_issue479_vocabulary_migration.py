...
class CoreVocabularyMigrationTests(TransactionTestCase):
    ...
    def _assert_exact_vocabulary(self, apps):
        ...
        for expected in expected_fields:
            ...
            if expected["choice_set"] is not None:
                ...
                self.assertEqual(
                    (choice_set.namespace, choice_set.slug), ("itambox", expected["choice_set"].split("/", 1)[1])
                )
            validation = expected["validation"]
            self.assertEqual(
                None if validation.get("minimum") is None else Decimal(validation["minimum"]), field.minimum_value
            )
            self.assertEqual(
                None if validation.get("maximum") is None else Decimal(validation["maximum"]), field.maximum_value
            )
        ...
        for expected in oracle["categories"]:
            ...
            self.assertEqual(
                sorted((key for key, value in category.applies_to.items() if value)), sorted(expected["applies_to"])
            )
    ...
    def test_supported_predecessor_upgrade_preserves_values_and_local_ownership(self):
        ...
        local_choice = Choice._base_manager.create(
            choice_set_id=local_choice_set.pk, key="local_value", label="Local value", position=10, lifecycle="active"
        )
        ...
        old_asset_type_membership = apps.get_model("assets", "AssetTypeFieldset")._base_manager.create(
            asset_type_id=asset_type.pk, fieldset_id=old_storage.pk, position=10
        )
        apps = self._migrate(self.target)
        self._assert_exact_vocabulary(apps)
