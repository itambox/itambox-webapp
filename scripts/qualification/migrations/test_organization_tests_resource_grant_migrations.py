"""Migration rehearsal from itambox/organization/tests/test_resource_grant_migrations.py — run explicitly:
...
class ResourceGrantExpiryMigrationTests(TransactionTestCase):
    ...
    def test_forward_and_reverse_states_are_schema_only(self):
        ...
        group_grantee = old_tenant_model.objects.create(name='Migration Group Grantee', slug='migration-group-grantee', group=group)
        resource_type = ContentType.objects.get_for_model(AccessoryStock)
        revoked_at = timezone.now()
        direct = old_grant(tenant=owner, grantee_tenant=grantee, resource_type_id=resource_type.pk, resource_id=1001, access_level='view')
        group_grant = old_grant(tenant=owner, grantee_tenant_group=group, resource_type_id=resource_type.pk, resource_id=1002, access_level='view')
        revoked = old_grant(tenant=owner, grantee_tenant=grantee, resource_type_id=resource_type.pk, resource_id=1003, access_level='view', deleted_at=revoked_at)
        ...
        first_run = TenantResourceGrantExpiryRun._base_manager.create(tenant_id=owner.pk, schedule_slot=timezone.now().replace(minute=0, second=0, microsecond=0), cutoff=timezone.now(), dispatch_stale_at=timezone.now() + timezone.timedelta(minutes=1))
        ...
        expired_grantee = TenantResourceGrant(tenant_id=owner.pk, grantee_tenant_id=group_grantee.pk, resource_type_id=resource_type.pk, resource_id=1004, access_level='view', valid_until=timezone.now())
        TenantResourceGrant._base_manager.bulk_create([expired_grantee])
        cutoff = timezone.now()
        expiry_run = TenantResourceGrantExpiryRun._base_manager.create(tenant_id=owner.pk, schedule_slot=cutoff.replace(minute=0, second=0, microsecond=0) + timezone.timedelta(hours=1), cutoff=cutoff, dispatch_stale_at=cutoff + timezone.timedelta(minutes=1))
