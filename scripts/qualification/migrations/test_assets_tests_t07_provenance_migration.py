"""Migration rehearsal from itambox/assets/tests/test_t07_provenance_migration.py — run explicitly:
...
MIGRATE_FROM = [('assets', '0114_issue479_t06_composition_schema'), ('extras', '0118_issue479_t06_definition_schema')]

MIGRATE_TO = [('assets', '0115_issue479_t07_provenance_bridge'), ('extras', '0120_issue479_t07_provenance_cutover')]
...
LOCAL_TYPE_PATHS = {'snipeit': {'source_url': 'https://snipe.example', 'source_id': '107'}, 'operator': {'note': 'retain', 'priority': 3}, 'archive': {'legacy': ['one', 'two']}}
...
class T07ProvenanceMigrationTests(IsolatedMigrationTestCase):
    ...
    def test_forward_preserves_ids_values_composition_tokens_and_archive(self):
        ...
        legacy_library = AssetTypeLibrary.objects.create(pk=700, namespace='legacy-stage', release='2026.09', source_checksum=LEGACY_LIBRARY_CHECKSUM, managed_paths=legacy_paths, last_reconciled_at=reconciled_at)
        empty_library = AssetTypeLibrary.objects.create(pk=701, namespace='empty-stage', release='', source_checksum=None, managed_paths={}, last_reconciled_at=None)
        manufacturer = Manufacturer.objects.create(pk=1700, name='T07 Migration Manufacturer', slug='t07-migration')
        fieldset = CustomFieldset.objects.create(pk=2700, namespace='local', slug='t07-migration-fieldset', label='T07 Migration Fieldset', description='Historical composition that must survive.', management_kind='local', version=1, lifecycle='active', managed_paths={'fieldset': {'retain': True}}, source_checksum=FIELDSET_CHECKSUM, last_reconciled_at=reconciled_at)
        field = CustomField.objects.create(pk=2701, name='t07_migration_field', namespace='local', label='T07 Migration Field', help_text='Historical field value.', field_type='text', activation='composed', management_kind='local', version=1, lifecycle='active', managed_paths={'field': {'retain': 'metadata'}}, source_checksum=FIELD_CHECKSUM, last_reconciled_at=reconciled_at)
        ...
        choice_set = CustomFieldChoiceSet.objects.create(pk=2702, namespace='local', slug='t07-migration-choice-set', label='T07 Migration Choice Set', management_kind='local', version=1, lifecycle='active', managed_paths={'choice_set': {'retain': 'metadata'}}, source_checksum=CHOICE_SET_CHECKSUM, last_reconciled_at=reconciled_at)
        choice = CustomFieldChoice.objects.create(pk=2703, choice_set_id=choice_set.pk, key='historical_value', label='Historical Value', position=1, management_kind='local', version=1, lifecycle='active', managed_paths={'choice_set': {'retain': 'metadata'}}, source_checksum=CHOICE_CHECKSUM, last_reconciled_at=reconciled_at)
        local_type = AssetType.objects.create(pk=1701, manufacturer_id=manufacturer.pk, model='Imported Local Type', slug='imported-local-type', management_kind='local', custom_field_data={'memory_capacity': '16.000', 'enabled': False, 'operator_note': 'retain'}, managed_paths=LOCAL_TYPE_PATHS)
        library_type = AssetType.objects.create(pk=1702, manufacturer_id=manufacturer.pk, model='Imported Library Type', slug='imported-library-type', management_kind='library', library_id=legacy_library.pk, library_definition_key='imported-library-type', library_release='2026.09', source_checksum=LEGACY_TYPE_CHECKSUM, managed_paths=LIBRARY_TYPE_PATHS, custom_field_data={'memory_capacity': '32.000', 'operator_note': 'retain'})
        ...
        self.assertEqual(migrated_local.custom_field_data, {'memory_capacity': '16.000', 'enabled': False, 'operator_note': 'retain'})
        self.assertEqual(migrated_local.connector_identity, TYPE_CONNECTOR_IDENTITY)
        self.assertEqual(list(NewAssetTypeFieldset.objects.filter(asset_type_id=local_type.pk).values_list('fieldset_id', 'position')), [(fieldset.pk, 7)])
        ...
        self.assertEqual(migrated_library.custom_field_data, {'memory_capacity': '32.000', 'operator_note': 'retain'})
        self.assertEqual(list(NewAssetTypeFieldset.objects.filter(asset_type_id=library_type.pk).values_list('fieldset_id', 'position')), [(fieldset.pk, 9)])
        ...
        self.assertEqual(list(migrated_fieldset.field_memberships.values_list('custom_field_id', 'position')), [(field.pk, 4)])
        ...
        for owner_kind, owner_id, checksum, managed_paths in (('custom_field', field.pk, FIELD_CHECKSUM, {'field': {'retain': 'metadata'}}), ('custom_fieldset', fieldset.pk, FIELDSET_CHECKSUM, {'fieldset': {'retain': True}}), ('choice_set', choice_set.pk, CHOICE_SET_CHECKSUM, {'choice_set': {'retain': 'metadata'}}), ('choice', choice.pk, CHOICE_CHECKSUM, {'choice_set': {'retain': 'metadata'}})):
