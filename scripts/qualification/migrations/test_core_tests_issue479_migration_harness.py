"""Migration rehearsal from itambox/core/tests/test_issue479_migration_harness.py — run explicitly:
...
from core.tests.migration_harness import IsolatedMigrationTestCase, _child_database_name, _child_environment, _child_pytest_args, _cleanup_child_database, _create_child_database, _emit_phase, _is_owned_database_name, _run_isolated_test, isolate_migration_tests
...
class MigrationHarnessContractTests(SimpleTestCase):
    ...
    def test_empty_child_database_has_no_sequences_to_reset(self):
        with patch('core.tests.migration_harness._is_isolated_child', return_value=True), patch.object(connection.introspection, 'table_names', return_value=[]), patch('django.test.TransactionTestCase._reset_sequences') as reset:
            IsolatedMigrationTestCase._reset_sequences('default')
        reset.assert_not_called()

    def test_populated_child_database_keeps_normal_sequence_reset(self):
        with patch('core.tests.migration_harness._is_isolated_child', return_value=True), patch.object(connection.introspection, 'table_names', return_value=['auth_group']), patch('django.test.TransactionTestCase._reset_sequences') as reset:
            IsolatedMigrationTestCase._reset_sequences('default')
    ...
    def test_child_environment_preserves_coverage_and_records_node(self):
        ...
        with patch.dict(os.environ, {'COVERAGE_FILE': '../artifacts/.coverage.serial', 'COVERAGE_PROCESS_START': 'pyproject.toml', 'PYTEST_ADDOPTS': '--maxfail=1', 'ITAMBOX_NODE_ID_MANIFEST': '../artifacts/parent-nodes.txt', 'ITAMBOX_NODE_ID_MANIFEST_WRITE': '1'}, clear=False):
    ...
    def test_phase_output_is_flushed_and_identifies_node_pid_and_timestamp(self):
        ...
        self.assertRegex(line, 'pid=\\d+')
        self.assertRegex(line, 'timestamp=\\d{4}-\\d{2}-\\d{2}T')
    ...
    def test_cleanup_timeout_is_reported(self):
        database = _child_database_name('core/tests/test_issue479_migration_harness.py::test_case', 101)
        with patch('core.tests.migration_harness.subprocess.run', side_effect=subprocess.TimeoutExpired('python', 30)):
            cleanup_error = _cleanup_child_database(database, os.environ.copy())
