"""Migration rehearsal from itambox/assets/tests/test_issue479_foundation_migrations.py — run explicitly:
...
class AssetTypeFoundationMigrationTests(IsolatedMigrationTestCase):
    migrate_from = [('assets', '0101_seed_canonical_missing_status'), ('extras', '0113_upgrade_legacy_webhook_retry_schedules')]
    ...
    def test_core_seed_creates_object_type_contenttypes_on_fresh_upgrade(self):
        ...
        for name, expected_models in (('processor_model', {'asset', 'assettype'}), ('hostname', {'asset'})):
    ...
    def test_core_seed_rejects_values_invalidated_by_core_contract(self):
        ...
        source_fields = (('ram_gb', 'RAM (GB)', 'number', asset_type_ct), ('storage_gb', 'Storage (GB)', 'number', asset_type_ct), ('poe_budget_w', 'PoE Budget (Watts)', 'number', asset_type_ct), ('hostname', 'Hostname', 'text', asset_ct), ('firmware_version', 'Firmware Version', 'text', asset_ct))
        ...
        AssetType.objects.create(manufacturer=manufacturer, model='Invalid Core Value Model', slug='invalid-core-value-model', custom_field_data={'analog_input_count': '1.5', 'operating_system_family': 'not-a-choice'})
        target = [('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')]
    ...
    def test_core_seed_rejects_malformed_decimal_text(self):
        ...
        AssetType.objects.create(manufacturer=manufacturer, model='Malformed Decimal Model', slug='malformed-decimal-model', custom_field_data={'memory_capacity': '1x000'})
        target = [('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')]
        with self.assertRaisesRegex(RuntimeError, 'issue479:core_value'):
            MigrationExecutor(connection).migrate(target)
    ...
    def test_core_seed_rejects_signed_zero_decimal_text(self):
        ...
        AssetType.objects.create(manufacturer=manufacturer, model='Signed Zero Model', slug='signed-zero-model', custom_field_data={'memory_capacity': '-0.000'})
        target = [('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')]
        with self.assertRaisesRegex(RuntimeError, 'issue479:core_value'):
            MigrationExecutor(connection).migrate(target)
    ...
    def test_core_seed_rejects_inverted_temperature_range(self):
        ...
        AssetType.objects.create(manufacturer=manufacturer, model='Inverted Temperature Model', slug='inverted-temperature-model', custom_field_data={'operating_temperature_min': '10.00', 'operating_temperature_max': '-10.00'})
        target = [('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')]
        with self.assertRaisesRegex(RuntimeError, 'issue479:core_value'):
            MigrationExecutor(connection).migrate(target)
    ...
    def test_fieldset_cutover_catches_up_post_0103_legacy_rows(self):
        executor = MigrationExecutor(connection)
        pre_cutover = [('assets', '0104_asset_type_composition_backfill'), ('extras', '0114_asset_type_definition_schema')]
        ...
        Manufacturer = old_apps.get_model('assets', 'Manufacturer')
        AssetType = old_apps.get_model('assets', 'AssetType')
        field = CustomField.objects.create(name='legacy_select_late', label='Late Legacy Select', field_type='select', choices='Production\nStaging')
        stable_field = CustomField.objects.create(name='legacy_select_stable', label='Stable Legacy Select', field_type='select', choices='stable')
        fieldset = CustomFieldset.objects.create(name='Late Legacy Fieldset', slug=None)
        manufacturer = Manufacturer.objects.create(name='Late Legacy Maker')
        asset_type = AssetType.objects.create(manufacturer=manufacturer, model='Late Legacy Model', slug='late-legacy-model', custom_field_data={'legacy_select_late': 'Production', 'legacy_select_stable': 'stable'})
        ...
        self.assertEqual(list(migrated_stable_field.choice_set.choices.values_list('label', flat=True)), ['stable'])
        self.assertEqual(list(migrated_field.choice_set.choices.values_list('label', flat=True)), ['Production', 'Staging'])
        ...
        self.assertEqual(list(migrated_fieldset.fields.values_list('name', flat=True)), ['legacy_select_late', 'legacy_select_stable'])
    ...
    def test_legacy_definitions_values_and_composition_are_preserved(self):
        ...
        for model in ('assettype', 'asset'):
            ContentType.objects.get_or_create(app_label='assets', model=model)
        CustomField = old_apps.get_model('extras', 'CustomField')
        CustomFieldset = old_apps.get_model('extras', 'CustomFieldset')
        Manufacturer = old_apps.get_model('assets', 'Manufacturer')
        AssetType = old_apps.get_model('assets', 'AssetType')
        ...
        poe = CustomField.objects.create(name='poe_budget_w', label='PoE Budget (Watts)', field_type='number', choices='')
        ...
        firmware = CustomField.objects.create(name='firmware_version', label='Firmware Version', field_type='text', choices='')
        firmware.object_types.add(ContentType.objects.get(app_label='assets', model='asset'))
        environment = CustomField.objects.create(name='environment', label='Environment', field_type='select', choices='Production\nStaging')
        ...
        asset_type = AssetType.objects.create(manufacturer=manufacturer, model='Server', slug='example-server', custom_fieldset=fieldset, custom_field_data={'ram_gb': 16, 'environment': 'Production', 'snipeit_id': 42})
        executor = MigrationExecutor(connection)
        ...
        self.assertEqual(list(migrated_environment.choice_set.choices.values_list('key', 'label', 'position')), [('production', 'Production', 10), ('staging', 'Staging', 20)])
        self.assertEqual((migrated_fieldset.namespace, migrated_fieldset.label), ('local', 'Issue 479 Migration Specs'))
        self.assertEqual(list(migrated_fieldset.field_memberships.values_list('custom_field__name', 'position')), [('memory_capacity', 10), ('environment', 20)])
        self.assertEqual(migrated_asset_type.custom_field_data, {'memory_capacity': '16.000', 'environment': 'production', 'snipeit_id': 42})
        self.assertEqual(list(migrated_asset_type.fieldset_memberships.values_list('fieldset_id', 'position')), [(fieldset.pk, 10)])
        executor = MigrationExecutor(connection)
        executor.migrate([('assets', '0105_asset_type_core_adoption'), ('extras', '0114_asset_type_definition_schema')])
        adopted_apps = executor.loader.project_state([('assets', '0105_asset_type_core_adoption'), ('extras', '0114_asset_type_definition_schema')]).apps
    ...
    def test_core_seed_refuses_local_field_identity(self):
        ...
        old_apps = executor.loader.project_state(pre_seed).apps
        CustomField = old_apps.get_model('extras', 'CustomField')
        CustomField.objects.create(name='form_factor', label='Local form factor', field_type='text', namespace='local', management_kind='local', version=1, lifecycle='active')
        with self.assertRaisesRegex(RuntimeError, 'issue479:core_field_identity_collision'):
            MigrationExecutor(connection).migrate([('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')])
    ...
    def test_core_seed_refuses_fieldset_identity(self):
        ...
        old_apps = executor.loader.project_state(pre_seed).apps
        CustomFieldset = old_apps.get_model('extras', 'CustomFieldset')
        CustomFieldset.objects.create(name='Local compute memory', namespace='itambox', slug='compute-memory', label='Local compute memory', management_kind='local', lifecycle='active')
    ...
    def test_core_seed_refuses_unexpected_fieldset_membership(self):
        ...
        fieldset = CustomFieldset.objects.create(name='Compute memory', namespace='itambox', slug='compute-memory', label='Compute memory', management_kind='core', lifecycle='active')
        unexpected = CustomField.objects.create(name='unexpected_field', label='Unexpected field', field_type='text', scope='asset_type', namespace='itambox', management_kind='core', lifecycle='active', version=1)
    ...
    def test_core_seed_allows_local_fieldset_same_slug(self):
        ...
        old_apps = executor.loader.project_state(pre_seed).apps
        CustomFieldset = old_apps.get_model('extras', 'CustomFieldset')
        CustomFieldset.objects.create(name='Local compute memory', namespace='local', slug='compute-memory', label='Local compute memory', management_kind='local', lifecycle='active')
        target = [('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')]
        MigrationExecutor(connection).migrate(target)
        migrated_apps = MigrationExecutor(connection).loader.project_state(target).apps
        CustomFieldset = migrated_apps.get_model('extras', 'CustomFieldset')
    ...
    def test_core_seed_refuses_fieldset_tombstone(self):
        ...
        old_apps = executor.loader.project_state(pre_seed).apps
        CustomFieldset = old_apps.get_model('extras', 'CustomFieldset')
        CustomFieldset.objects.create(name='Deleted compute memory', namespace='itambox', slug='compute-memory', label='Deleted compute memory', management_kind='local', lifecycle='deleted', deleted_at=timezone.now())
        with self.assertRaisesRegex(RuntimeError, 'issue479:core_fieldset_identity_collision'):
            MigrationExecutor(connection).migrate([('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')])
    ...
    def test_core_seed_allows_local_choice_set_same_slug(self):
        ...
        ChoiceSet.objects.create(namespace='local', slug='form-factor', label='Local form factor', management_kind='local', lifecycle='active')
        target = [('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')]
        MigrationExecutor(connection).migrate(target)
        migrated_apps = MigrationExecutor(connection).loader.project_state(target).apps
    ...
    def test_core_seed_refuses_field_lifecycle_collision(self):
        ...
        ChoiceSet = old_apps.get_model('extras', 'CustomFieldChoiceSet')
        CustomField = old_apps.get_model('extras', 'CustomField')
        choice_set = ChoiceSet.objects.create(namespace='itambox', slug='form-factor', label='Form factor', management_kind='core', lifecycle='active')
        CustomField.objects.create(name='form_factor', label='Form factor', field_type='single-select', scope='asset_type', namespace='itambox', choice_set=choice_set, management_kind='core', lifecycle='deprecated', version=1)
        with self.assertRaisesRegex(RuntimeError, 'issue479:core_field_identity_collision'):
            MigrationExecutor(connection).migrate([('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')])
    ...
    def test_core_seed_refuses_choice_lifecycle_collision(self):
        ...
        choice_set = ChoiceSet.objects.create(namespace='itambox', slug='form-factor', label='Form factor', management_kind='core', lifecycle='active')
        Choice.objects.create(choice_set=choice_set, key='notebook', label='Notebook', position=999, management_kind='core', lifecycle='deprecated')
    ...
    def test_core_seed_refuses_choice_set_tombstone(self):
        ...
        old_apps = executor.loader.project_state(pre_seed).apps
        ChoiceSet = old_apps.get_model('extras', 'CustomFieldChoiceSet')
        ChoiceSet.objects.create(namespace='itambox', slug='form-factor', label='Reserved', management_kind='local', lifecycle='deleted', deleted_at=timezone.now())
        with self.assertRaisesRegex(RuntimeError, 'issue479:core_choice_set_collision'):
            MigrationExecutor(connection).migrate([('assets', '0106_asset_type_core_seed'), ('extras', '0114_asset_type_definition_schema')])

    def test_asset_type_cutover_reverse_is_explicitly_refused(self):
        executor = MigrationExecutor(connection)
        executor.migrate([('assets', '0108_asset_type_singular_cutover'), ('extras', '0115_asset_type_fieldset_cutover')])
        with self.assertRaisesRegex(RuntimeError, 'issue479:reverse_refused'):
            MigrationExecutor(connection).migrate([('assets', '0107_asset_type_library_contract'), ('extras', '0115_asset_type_fieldset_cutover')])

    def test_fieldset_cutover_reverse_is_explicitly_refused(self):
        executor = MigrationExecutor(connection)
        executor.migrate([('assets', '0107_asset_type_library_contract'), ('extras', '0115_asset_type_fieldset_cutover')])
        with self.assertRaisesRegex(RuntimeError, 'issue479:reverse_refused'):
            MigrationExecutor(connection).migrate([('assets', '0107_asset_type_library_contract'), ('extras', '0114_asset_type_definition_schema')])
