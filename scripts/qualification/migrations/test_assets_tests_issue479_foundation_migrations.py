...
class AssetTypeFoundationMigrationTests(IsolatedMigrationTestCase):
    ...
    def test_core_seed_rejects_inverted_temperature_range(self):
        ...
        AssetType.objects.create(
            ...
            custom_field_data={"operating_temperature_min": "10.00", "operating_temperature_max": "-10.00"},
        )
        target = [("assets", "0106_asset_type_core_seed"), ("extras", "0114_asset_type_definition_schema")]
        with self.assertRaisesRegex(RuntimeError, "issue479:core_value"):
            MigrationExecutor(connection).migrate(target)
    ...
    def test_fieldset_cutover_catches_up_post_0103_legacy_rows(self):
        ...
        field = CustomField.objects.create(
            name="legacy_select_late", label="Late Legacy Select", field_type="select", choices="Production\nStaging"
        )
        stable_field = CustomField.objects.create(
            name="legacy_select_stable", label="Stable Legacy Select", field_type="select", choices="stable"
        )
        ...
        self.assertEqual(
            list(migrated_field.choice_set.choices.values_list("label", flat=True)), ["Production", "Staging"]
        )
    ...
    def test_legacy_definitions_values_and_composition_are_preserved(self):
        ...
        environment = CustomField.objects.create(
            name="environment", label="Environment", field_type="select", choices="Production\nStaging"
        )
        ...
        self.assertEqual(
            list(migrated_asset_type.fieldset_memberships.values_list("fieldset_id", "position")), [(fieldset.pk, 10)]
        )
        executor = MigrationExecutor(connection)
    ...
    def test_core_seed_refuses_field_lifecycle_collision(self):
        ...
        choice_set = ChoiceSet.objects.create(
            namespace="itambox", slug="form-factor", label="Form factor", management_kind="core", lifecycle="active"
        )
        CustomField.objects.create(
            name="form_factor",
            label="Form factor",
