"""Migration rehearsal from itambox/users/tests/test_oidc_identity_migrations.py — run explicitly:
...
class OIDCIdentityMigrationTests(TransactionTestCase):
    ...
    def test_forward_and_reverse_preserve_predecessor_users_without_backfill(self):
        ...
        unique_pairs = [constraint for constraint in constraints.values() if constraint['unique'] and constraint['columns'] == ['issuer', 'subject']]
        ...
        binding = OIDCIdentity.objects.create(user_id=predecessor_user_pk, issuer='https://migration.example/issuer', subject='migration-subject')
