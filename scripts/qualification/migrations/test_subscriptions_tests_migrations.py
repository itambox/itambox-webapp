"""Migration rehearsal from itambox/subscriptions/tests/test_migrations.py — run explicitly:
...
class SubscriptionVocabularyMigrationTests(TransactionTestCase):
    ...
    def test_legacy_statuses_and_auto_renewal_values_are_preserved(self):
        ...
        rows = [Subscription.objects.create(name=f'Legacy {status}', provider=provider, status=status, auto_renewal=auto_renewal) for status, auto_renewal in (('pending', False), ('trial', True), ('renewing', False))]
        executor = MigrationExecutor(connection)
        ...
        self.assertEqual([row.vendor_contract_auto_renews for row in migrated], [False, True, False])
        executor = MigrationExecutor(connection)
        executor.migrate([self.migrate_from])
