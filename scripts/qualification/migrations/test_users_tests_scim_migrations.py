"""Migration rehearsal from itambox/users/tests/test_scim_migrations.py — run explicitly:
...
class SCIMIdentityMigrationTests(IsolatedMigrationTestCase):
    migrate_from = [('organization', '0100_issue88_shard_61_organization_relations'), ('users', '0100_issue88_shard_62_users_relations')]
    migrate_to = [('organization', '0101_membership_external_id_and_more'), ('users', '0101_user_scim_id_usergroup_external_id_usergroup_scim_id_and_more')]
    ...
    def test_forward_populates_ids_and_reverse_preserves_existing_rows(self):
        ...
        group_membership = GroupMembership.objects.create(user_group=group, membership=membership, source='scim', external_id='legacy-provenance')
        extra_group_membership = GroupMembership.objects.create(user_group=extra_group, membership=extra_membership, source='manual', external_id='legacy-extra-provenance')
        user_pks = [user.pk, extra_user.pk]
        group_pks = [group.pk, extra_group.pk, deleted_group.pk]
        user_pk, group_pk, membership_pk, group_membership_pk = (user.pk, group.pk, membership.pk, group_membership.pk)
