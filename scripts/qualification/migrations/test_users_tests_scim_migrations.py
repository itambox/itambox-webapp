...
class SCIMIdentityMigrationTests(IsolatedMigrationTestCase):
    ...
    def test_forward_populates_ids_and_reverse_preserves_existing_rows(self):
        ...
        group_membership = GroupMembership.objects.create(
            user_group=group, membership=membership, source="scim", external_id="legacy-provenance"
        )
        extra_group_membership = GroupMembership.objects.create(
            user_group=extra_group, membership=extra_membership, source="manual", external_id="legacy-extra-provenance"
        )
