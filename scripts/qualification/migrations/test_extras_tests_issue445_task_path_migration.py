...
class Issue445TaskPathMigrationTests(IsolatedMigrationTestCase):
    ...
    def test_complete_forward_reverse_forward_lifecycle(self):
        try:
            ...
            for old_path, new_path in TASK_PATH_MAP.items():
                ...
                self.assertEqual(
                    NewSchedule.objects.filter(func=new_path).count(), expected, f"multiplicity drift on {new_path}"
                )
            for pk, old_row in seeded.items():
                ...
                for field in NON_FUNC_FIELDS:
                    self.assertEqual(
                        getattr(new_row, field), getattr(old_row, field), f"field {field} drifted on schedule {pk}"
                    )
    ...
    def test_post_migrate_alert_registration_only_after_cutover_and_absent_on_reverse(self):
        try:
            ...
            post_migrate.send(
                sender=extras_config, app_config=extras_config, verbosity=0, interactive=False, using=connection.alias
            )
            post_migrate.send(
                sender=extras_config, app_config=extras_config, verbosity=0, interactive=False, using=connection.alias
            )
