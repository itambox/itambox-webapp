...
from extras.tasks.webhooks import WebhookDeliveryAssertions, _decrypt_target_secret, _encrypted_secret_snapshot, _endpoint_target_snapshot, _finish_delivery, redeliver_webhook_delivery, send_webhook_task, send_webhook_test
...
class WebhookDeliveryMigrationTests(IsolatedMigrationTestCase):
    ...
    def test_forward_reverse_reapply_has_no_backfill_operation(self):
        ...
        old_extras_models = {model._meta.model_name for model in old_apps.get_models() if model._meta.app_label == 'extras'}
...
class WebhookDeliveryTargetMigrationTests(IsolatedMigrationTestCase):
    ...
    def test_forward_reverse_reapply_backfills_only_endpoint_targets(self):
        ...
        endpoint = Endpoint.objects.create(name='Migration endpoint', url='https://example.invalid/migration-target', http_method='PATCH', headers={'X-Migration': 'header'}, secret='enc$migration-ciphertext', retry_count=7, retry_backoff=23)
        linked = Delivery.objects.create(endpoint_id=endpoint.pk, delivery_id=str(uuid4()), status='pending')
        plaintext_endpoint = Endpoint.objects.create(name='Legacy plaintext migration endpoint', url='https://example.invalid/plaintext-target', secret='legacy-plaintext-secret')
        plaintext = Delivery.objects.create(endpoint_id=plaintext_endpoint.pk, delivery_id=str(uuid4()), status='pending')
...
class WebhookRetryScheduleMigrationTests(IsolatedMigrationTestCase):
    ...
    def test_forward_executes_legacy_retry_and_reverse_is_refused(self):
        ...
        legacy = {'url': 'https://example.invalid/legacy-retry', 'method': 'POST', 'headers': {'Authorization': 'Bearer legacy-header-secret'}, 'secret': 'legacy-hmac-secret', 'webhook_endpoint_id': None, 'event_id': None, 'delivery_id': delivery.delivery_id, 'tenant_id': None, 'event_action': 'test', 'event_model_app_label': 'extras', 'event_model_name': 'webhookendpoint', 'event_object_id': 1, 'event_timestamp_iso': '2026-01-01T00:00:00+00:00', 'event_data': {}, 'attempt': 1, 'retry_count': 3, 'retry_backoff': 60, 'actor_id': None, 'request_id': 'issue445-migration', 'test_send': True}
        schedule = HistoricalSchedule.objects.create(name='Legacy webhook retry', func='extras.tasks.webhooks.send_webhook_task', kwargs=repr(legacy), schedule_type='O', repeats=1)
        non_kwargs = {field: getattr(schedule, field) for field in ('pk', 'name', 'func', 'schedule_type', 'repeats', 'hook', 'args')}
        new_apps = self._migrate(self.migrate_to).apps
        ...
        with patch('core.http.request_pinned', return_value=response) as request_pinned:
            result = send_webhook_task(parsed['assertions'], attempt=parsed['attempt'], actor_id=parsed['actor_id'], request_id=parsed['request_id'])
    ...
    def test_malformed_legacy_retry_payload_fails_closed(self):
        old_apps = self._migrate(self.migrate_from).apps
        HistoricalSchedule = old_apps.get_model('django_q', 'Schedule')
        HistoricalSchedule.objects.create(name='Malformed legacy webhook retry', func='extras.tasks.webhooks.send_webhook_task', kwargs=repr({'url': 'https://example.invalid/leaks-secret', 'secret': 'must-not-appear'}), schedule_type='O', repeats=1)
